...
MotorModule.prototype.setRightWheelSpeed = function(speed){
	if(maestro){
		...
		console.log("setting right wheel: " + speed);
		maestro.setTarget(SERVO_MAP.right, speed);		
	}
}
...
MotorModule.prototype.setLeftWheelSpeed = function(speed){
	if(maestro){
		...
		console.log("setting left wheel: " + servoInverse);
		maestro.setTarget(SERVO_MAP.left, servoInverse);	
	}
}

MotorModule.prototype.turnRight = function(){
	this.setLeftWheelSpeed(leftWheelSpeed + 10);
	this.setRightWheelSpeed(rightWheelSpeed - 10);
}

MotorModule.prototype.turnLeft = function(){
	this.setLeftWheelSpeed(leftWheelSpeed - 10);
	this.setRightWheelSpeed(rightWheelSpeed + 10);
}
